...
module.exports = async function warmupItemsCache({ fastify, mongoClient, redisClient }) {
  const startGlobal = Date.now();
  const log = fastify.log;
  const { ITEMS_KEY_PREFIX } = require('./itemsCacheKey');
  const col = mongoClient.db('TORN').collection('Items');
