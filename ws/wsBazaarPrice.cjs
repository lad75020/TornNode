...
async function fetchSingleName(fastify, id) {
  ...
  const doc = await fastify.mongo.client.db('TORN')
      .collection('Items')
...
async function updatePriceStores(fastify, itemId, price, itemName) {
  if (!fastify.mongo) return;
  try {
  const db = fastify.mongo.client.db('TORN'); // Items globaux
...
async function loadItemNames(fastify) {
  ...
  if (fastify.mongo) {
    try {
  const items = await fastify.mongo.client.db('TORN').collection('Items')
...
async function seedWatchSetFromMongo(fastify) {
  ...
  try {
    const itemsCol = fastify.mongo.client.db('TORN').collection('Items');
