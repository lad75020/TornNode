...
async function updatePrice(){
    ...
    .then(data => {
        ...
        if (document.getElementById(`item_${itemID}`)) {
            document.getElementById(`item_${itemID}`).innerHTML = `<img src="${data.imageURL}" style="width: 100px; height: 100%;">${data.name}&nbsp;$&nbsp;${data.price}`;
        }
        initAutoComplete();
    });
}
...
async function prepareData(chartName){
    ...
    if (currentChart) {
        ...
        if(currentChart.name == "Faction Balance"){
            ...
            await retrieveLogsByLog(currentChart.log, first, last + DAY_TO_SEC).then(objects => {
                let sum = 0;
                for (balance of objects) {
                    if(balance.data.balance_after > balance.data.balance_before){
                        sum += balance.data.balance_after - balance.data.balance_before;
                    data2.push([new Date(balance.timestamp * 1000), sum,'color:green']);
                    }
                }
            });
            return (data2);
        ...
        for (t=first; t<last+DAY_TO_SEC; t += DAY_TO_SEC){
            ...
            if (log != 9005 && log != 1403 && log != 5410 && log != 1112 && log !=2290 && log != 1226 && log !=6738 && log!= 8731 && log!= 4810 && log != 2340 && log != 6000 && log != 5510 && category != "Gym" && type != "Attack" && type != "Trains" && type != "AllSkills" && type != "graffiti" && type != "Casino"){
            ...
            if(currentChart.name == "Items Dumped"){
                await retrieveLogsByLog(currentChart.log, t, t + DAY_TO_SEC).then(objects => {let count = 0; for (dumped of objects) {count += dumped.data.items[0].qty;} i.push(count);});
                i.push('color: blue');
                data1.push(i);
            }
            if ( type == "graffiti"){
            ...
            if (log == 8731){
                for (const result of ['win', 'lose']){
                        ...
                        .then(data => { 
                            if(result=='win'){
                                i.push((parseInt(data)) || 0);
                            }
                            else if (result == 'lose'){
                                i.push((parseInt(data)* -1) || 0);
