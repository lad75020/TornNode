module.exports = async function wsGetAllTornItems(socket, req, client, fastify, parsed) {
  ...
  const redisClient = fastify.redisClient || fastify.redis || fastify._redisClient;
  const { ITEMS_KEY_PREFIX, REQUIRED_ITEM_FIELDS } = require('../utils/itemsCacheKey');
  try {
    let docs;
    if (redisClient) {
    ...
    if (docs && !isIncomplete) {
      ...
      fastify.log.info('[wsGetAllTornItems] Incomplete cache -> reload Mongo');
    }
